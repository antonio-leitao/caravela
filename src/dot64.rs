...
pub fn ddot(a: &[f64], b: &[f64]) -> f64 {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            return unsafe { ddot_avx2_fma(a, b) };
        }
    }
...
unsafe fn ddot_avx2_fma(a: &[f64], b: &[f64]) -> f64 {
    let len = a.len();
    ...
    sum + ddot_scalar(&a[remainder_start..], &b[remainder_start..])
}

#[cfg(target_arch = "aarch64")]
#[target_feature(enable = "neon")]
unsafe fn ddot_neon(a: &[f64], b: &[f64]) -> f64 {
    let len = a.len();
...
#[inline(always)]
fn ddot_scalar(a: &[f64], b: &[f64]) -> f64 {
    let mut sum0 = 0.0;
    let mut sum1 = 0.0;
    let mut sum2 = 0.0;
    let mut sum3 = 0.0;
...
mod tests {
    ...
    fn test_dot_product_consistency_small() {
        ...
        assert_abs_diff_eq!(ddot(&a, &b), 32.0, epsilon = 1e-10);
    }
    ...
    fn test_dot_specialized_vs_general() {
        ...
        let simd_result = ddot(&a, &b);
        let scalar_result = ddot_scalar(&a, &b);
    ...
    fn test_dot_edge_cases() {
        ...
        assert_abs_diff_eq!(ddot(&zeros, &zeros), 0.0, epsilon = 1e-10);

        // Denormals
        let denormals = vec![f64::MIN_POSITIVE; 100];
        let expected = (f64::MIN_POSITIVE * f64::MIN_POSITIVE) * 100.0;
        assert_abs_diff_eq!(ddot(&denormals, &denormals), expected, epsilon = 1e-10);

        // Mixed signs
        let a = vec![1.0, -1.0, 2.0, -2.0];
        let b = vec![1.0, 1.0, 1.0, 1.0];
        assert_abs_diff_eq!(ddot(&a, &b), 0.0, epsilon = 1e-10);
    }
